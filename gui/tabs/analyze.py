...
def create_analyze_tab():
    with gr.TabItem("Analyze", id="Analyze"):
        gr.Markdown("### 🔍 Advanced Diagnostics (S3-DiT)")
        ...
        with gr.Row():
                ...
                with gr.Tabs():
                    with gr.Tab("Spectrum"): 
                        plot_spectrum = gr.LinePlot(x="Rank", y="LogValue", title="SVD Info Density", height=300, 
                                                   tooltip=["Rank", "Value"])
                        gr.Markdown("<small>Shows how much unique information is in each rank. A sharp drop indicates efficient compression potential.</small>")
                    with gr.Tab("Region Energy"): 
                        plot_energy = gr.BarPlot(x="Region", y="Energy", title="Block Energy Distribution", height=300)
                        gr.Markdown("<small>IN: Structure | MID: Concepts | OUT: Style. Imbalance leads to 'stubborn' or 'noisy' models.</small>")
                    with gr.Tab("Layer Heatmap"): 
                        plot_heatmap = gr.Image(label="Attention/MLP Activity Matrix", interactive=False)
                        gr.Markdown("<small>Vertical: 30 Blocks. Horizontal: Q, K, V, O, Gate, Up, Down. Bright spots = High activity/Overtraining.</small>")
            ...
            with gr.Column(scale=2):
                ...
                with gr.Group():
                    ...
                    rec_drop = gr.Dropdown(label="Recommendations", choices=[], info="Automated fixes based on Z-Image Turbo stability rules.")
                    fix_btn = gr.Button("Apply Fix & Open Morph", variant="secondary")
    
    # Return dict for wiring in layout
    return {
